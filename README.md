# SADRA
## Julia (based on PowerModels) and AIMMS Implementation of a Universal AC/DC Branch Model for Power Flow and Optimal Power Flow Studies
...
|--------|----------------|-------------|
| [`julia/`](julia/) | Julia / PowerModels | A package built on [PowerModels.jl](https://github.com/lanl-ansi/PowerModels.jl), solving both AC/DC optimal power flow and (from v1.1) AC/DC power flow, with full VSC control modes and controlled transformers, validated against the AIMMS reference and PowerModelsACDC. |
| [`aimms/`](aimms/) | AIMMS | The original reference implementation in AIMMS. |

### Julia / PowerModels implementation (`julia/`)

A PowerModels.jl implementation of SADRA for hybrid AC/DC optimal power flow and power flow, including the full set of converter control actions. It transforms a case (in PowerModelsACDC format, or the MATPOWER-FUBM format used by the paper's 1354-bus PEGASE case) into an augmented pure-AC network and solves it with the standard PowerModels machinery.
...
# AC/DC power flow (v1.1): fixed dispatch, converter controls as the PF specification
result = solve_sadra_pf("case5_acdc.m", opt)

println(result["objective"])
```

The Julia implementation has been validated against two independent references: it reproduces the **AIMMS reference** on the controlled 1354-bus PEGASE case (all five VSC control modes plus controlled phase-shifting and tap-changing transformers, objective 74,037.8), and reproduces **PowerModelsACDC**'s uncontrolled AC/DC OPF to within 0.003% on the 3120-bus benchmark. The v1.1 power flow is validated against PowerModelsACDC's AC/DC power flow (bus-voltage agreement to ~1e-4, with the residual attributed to the converter-station representation) and internally by reproducing OPF states to machine precision. See [`julia/VALIDATION.md`](julia/VALIDATION.md) for the full numbers, reproduction steps and limitations, and [`julia/README.md`](julia/README.md) for usage.
